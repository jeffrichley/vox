...
**Dependencies:** faster-whisper, sounddevice, pyperclip, pynput (or equivalent chosen in plan), Rich; Python ≥3.12, uv
...
- `.ai/REF/testing-and-gates.md` — Baseline loop and final gate (`just quality && just test`).
...
- **Acceptance gates:** `just quality && just test`; `uv run vox devices` exits 0 and prints device list; `uv run vox test-mic --seconds 2` records then plays back without error (no transcribe yet).
...
- [x] VALIDATE: `just quality && just test`; `uv run vox devices`; `uv run vox test-mic --seconds 2`.
...
- **Acceptance gates:** Unit test with fixture audio → transcribe → assert text; `vox test-mic --seconds 3` runs capture → play back → transcribe and prints text; `just quality && just test`.
...
- [x] VALIDATE: `just quality && just test`; run `vox test-mic --seconds 3` and confirm printed text.
...
- **Acceptance gates:** `vox run` starts and registers hotkey; manual test: focus text field, press hotkey, speak, release → text in clipboard and optionally in field; integration test simulates trigger and verifies clipboard content; `just quality && just test`.
...
- [ ] VALIDATE: `just quality && just test`; manual: `vox run`, hotkey, speak, release, check clipboard (and optional focused field).
...
- **Must:** README with install, config, run instructions; example config file; Rich tables/panels for `vox devices` and any list output; final gate `just quality && just test`; "Definition of Visible Done" satisfiable.
...
- **Acceptance criteria:** (1) README covers install, config location, env overrides, `vox run`/`vox devices`/`vox test-mic`, model setup, OS permissions, Definition of Visible Done. (2) `vox.toml.example` exists with hotkey, device_id, model_size, compute_type, injection_mode and comments. (3) `vox devices` (and any list) uses Rich table/panel; no raw JSON default. (4) Warnings from `just quality`/`just test` fixed or documented with rationale. (5) `docs/dev/status.md` (or equivalent) updated for MVP complete. (6) New user can complete Definition of Visible Done using README. (7) `just quality && just test` passes.
- **Non-goals:** New features or new code paths; agent wiring; tray/installer. Only polish, docs, and status.
- **Acceptance gates:** New user can follow README to install (uv), configure (hotkey, optional device/model), run `vox run`, and complete push-to-talk successfully; `just quality && just test`; all acceptance criteria below met.
...
- [ ] REVIEW warnings: run `just quality` and `just test`; fix or document any accepted residual warnings with rationale and owner/target.
- [ ] ADD or UPDATE `docs/dev/status.md` (or roadmap) per `.ai/REF/status-surfaces.md` to reflect MVP complete.
- [ ] VALIDATE: Full pass `just quality && just test`; walk through README as new user; confirm Definition of Visible Done.
...
| **Must not** | Silent fallbacks for missing config; hardcoded device or paths; skip validation; test-mic without play-back |
| **Exact acceptance gates** | `just quality && just test`; `uv run vox devices` (exit 0, device list); `uv run vox test-mic --seconds 2` (exit 0, record then play back, no transcribe) |
...
| **Exact acceptance gates** | Unit test: fixture audio → transcribe → assert text; `vox test-mic --seconds 3` runs capture + transcribe and prints text; `just quality && just test` |
...
| **Exact acceptance gates** | `vox run` starts and registers hotkey; manual: focus field, hotkey, speak, release → text in clipboard (and optionally field); integration test: trigger → verify clipboard; `just quality && just test` |
...
| **Must** | README (install, config, run); example config file; Rich for `vox devices`/lists; `just quality && just test`; Definition of Visible Done satisfiable |
| **Must not** | CLI as raw print only; ship without README or config example |
| **Exact acceptance gates** | New user follows README → install, configure, `vox run`, push-to-talk success; `just quality && just test`; all plan Acceptance criteria met |
...
8. **VALIDATE** `just quality && just test`.
...
13. **UPDATE** README — Model download, GPU/CPU, int8. **VALIDATE** `just quality && just test`.
...
19. **ADD** integration test: mock capture+transcribe, inject to clipboard, assert clipboard equals transcribed string. **VALIDATE** `just quality && just test`.
...
23. **RUN** `just quality && just test`; fix or document warnings. **VALIDATE** Clean or documented.
...
- `just types`
- `just quality` (format-check, lint, types, complexity, vulture, darglint, audit, bandit, radon, find-dupes, docstr-coverage, docs-check)
...
- **Generated during this feature:** Example config `vox.toml.example`; fixture audio for tests (in repo or generated in test).
...
When all phases are done, the following PRD MVP requirements are satisfied (no gaps):

| Status | PRD MVP requirement | Plan phase / task |
|--------|---------------------|-------------------|
| Pending | Push-to-talk (hold or toggle key) | Phase 3: hotkey on_press/on_release, capture start/stop |
| Pending | Local speech capture (default or configured mic) | Phase 1: capture module, config device_id; Phase 3: used in run loop |
| Pending | Local transcription with faster-whisper | Phase 2: transcribe module, model from config |
| Pending | Text injection: clipboard (min) | Phase 3: inject/clipboard.py, wired in run |
| Pending | Text injection: optional paste/type into focused window | Phase 3: inject/keystroke.py (optional), config injection_mode |
| Pending | Global hotkey, always-available | Phase 3: hotkey/register.py, vox run loop |
| Pending | CLI: vox run, vox devices, vox test-mic | Phase 1: devices, test-mic; Phase 3: run |
| Pending | Config from file and/or env; no silent fallbacks | Phase 1: config.py validation; all phases use config |
| Pending | Clear, actionable errors (mic, model, injection) | Phase 3: error handling task; config validation Phase 1 |
| Pending | Python 3.12+, uv, src/vox layout | Phase 1: package layout, pyproject |
| Pending | Quality gates: just quality && just test | All phases: VALIDATE at end; Completion checklist |
| Pending | README and config example | Phase 4: README, vox.toml.example |
| Pending | Rich CLI output (tables/panels); no raw JSON default | Phase 1: devices table; Phase 4: ensure Rich |

**Plan completion ⇒ entire MVP ready to use.**
...
- [ ] `just quality && just test` passes with no unresolved warnings (or documented with rationale).
...
- `just quality` — passed (format, lint, mypy, xenon, vulture, darglint, pip-audit, bandit, radon, pylint duplicate-code, docstr-coverage, docs-check).
- `just test` — 13 passed (tests/unit/test_config.py, tests/unit/test_capture.py).
...
- `justfile` — darglint: `-c Exit -i get_config` (CLI typer.Exit; get_config raises ValueError via validate_config)
...
- darglint: `typer.Exit` not documented in CLI (ignored with `-c Exit`); `get_config` skipped with `-i get_config` because it raises ValueError indirectly via `validate_config`.
...
**Commands run and outcomes:**
- `just quality` — passed.
- `just test` — 18 passed (added tests/unit/test_transcribe.py, tests/integration/test_capture_transcribe.py).
...
- `justfile` — darglint -i "get_config|transcribe" (transcribe raises RuntimeError via load_model).
