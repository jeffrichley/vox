---
description: "Create comprehensive feature plan with deep codebase analysis and research (canonical)"
---
...
- Include exact output artifacts and verification commands for user-visible functionality.
- Include a "Definition of Visible Done" section describing what a human can directly run/open/inspect.
...
- Find relevant documentation in `docs/`, `.ai/REF/`, and repository-local guidance files
...
```bash
PLAN_FILE=".ai/PLANS/<NNN>-<feature>.md"
...
### Intent Lock (Required For Each Phase)

For each implementation phase above, add an intent lock block before step-by-step tasks:

- Source of truth docs for that phase (specific file + section)
- `Must` non-negotiables
- `Must Not` forbidden shortcuts/defaults/fallbacks
- Provenance map for key output fields/decisions
- Acceptance gates (tests/commands that prove compliance)
...
- Exact output artifacts/surfaces:
  - `<artifact path | endpoint | screen | report>`
  - `<artifact path | endpoint | screen | report>`
...
- [ ] Output artifacts and verification commands are present (for user-visible features)
- [ ] "Definition of Visible Done" is present and testable (for user-visible features)
...
**Filename**: `.ai/PLANS/{NNN}-{kebab-case-descriptive-name}.md`

- Replace `{NNN}` with the next numeric plan ID and `{kebab-case-descriptive-name}` with a short, descriptive feature name
- Examples: `003-create-rules-codex-cursor.md`, `004-command-docs-agents-alignment.md`

**Directory**: Use existing `.ai/PLANS/`
...
- [ ] Output contract and visible-done proof are specific and directly executable/verifiable
