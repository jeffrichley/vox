...
[![CI](https://github.com/jeffrichley/vox/actions/workflows/ci.yml/badge.svg)](https://github.com/jeffrichley/vox/actions/workflows/ci.yml)
[![codecov](https://codecov.io/gh/jeffrichley/vox/branch/main/graph/badge.svg)](https://codecov.io/gh/jeffrichley/vox)
[![PyPI](https://img.shields.io/pypi/v/vox-core.svg)](https://pypi.org/project/vox-core/)
...
- **`vox`** or **`vox run`** — Start push-to-talk and Continuous dictation. By default a small window with a **Stop** button appears; with `use_tray = true` in config (or `VOX_TRAY=1`), a system tray icon is shown instead—click the icon and choose **Quit** to stop. Audible start/end recording cues are preloaded during startup so the first hotkey cycle does not stall on cue decode. Press and hold your configured hotkey to hear the start cue and begin recording, then release to hear the end cue while transcription/injection continues according to `injection_mode`: clipboard only, clipboard then paste, or direct typing into the focused window. With `clipboard_and_paste`, Vox snapshots your previous clipboard text, pastes the transcription, waits ~150 ms, then restores the snapshot when it is still safe to do so (skipped for empty/non-text clipboards, paste failures, or if you copied something new during the delay). Restore is text-only; images and files on the clipboard cannot be preserved.
...
- **Idle auto-off:** If Continuous stays on with no speech for `continuous_idle_minutes` (default `5`; env `VOX_CONTINUOUS_IDLE_MINUTES`; must be a finite number `> 0`), Vox turns it off the same way a toggle does (pending Commits still land), plays the end cue, and prints a dim idle message.
- **Failure handling:** Continuous never fails silently. In `clipboard` Injection mode it refuses to start (red message, end cue, published error) because each Commit would overwrite the last. If speech detection or the microphone stream cannot start, or if the mic stops delivering audio for 2 seconds while Continuous is on, it turns off with a red message, the end cue, and a published error state (pending Commits still land). A transcription error on one Commit prints a red message and leaves the session running. Cue, state-publish, and delivery exceptions are reported and never kill the listening or Commit threads.
- **Junk guard:** Utterances with under 0.3 s of detected speech (coughs, clicks, breaths) are discarded without transcription; a dim message shows the speech duration. Continuous transcription also drops Whisper segments with `no_speech_prob > 0.6` so non-speech hallucinations (for example "Thank you.") are not Injected. If filtering leaves nothing, a dim "No speech detected." is printed and nothing is Injected. Push-to-talk transcription is unchanged.
